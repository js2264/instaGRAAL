    ...
    def full_em(self, n_cycles, n_neighbours, bomb, id_start_sample_param, save_matrix=False):
        ...
        for j in range(0, n_cycles):
            ...
            try:
                ...
                logger.warning("Warning, could not write output files at {}: {}".format(j, e))
            try:
                if save_matrix:
...
def run_instagraal(
    ...
    if not pyramid_only:
        ...
        log_assembly_stats(p2.simulation.new_fasta, label="Scaffolded assembly")
    ...
    if save_pickle:
        try:
            ...
            logger.warning("Could not pickle state (likely due to h5py handles): {}".format(e))
